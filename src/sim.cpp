...
namespace madEscape
{
    ...
    void Sim::registerTypes(ECSRegistry &registry, const Config &cfg)
    {
        ...
        registry.registerComponent<PortalState>();
        ...
        registry.registerArchetype<PortalEntity>();
    ...
    inline void dummySystem(Engine &ctx, Rotation& rot) {
        Room* rooms = ctx.singleton<LevelState>().rooms;
        for (int i = 0; i < consts::numRooms; i++) {
            Room& room = rooms[i];
            Entity& door = room.door;
            // printf("Door ID for Room %d is %d!\n", i, door.id);
        }
    }
    ...
    // Checks if there is an entity standing on the portal and updates
    // PortalState if so.
    inline void portalSystem(Engine &ctx,
                              Position pos,
                              PortalState &state)
    {
        AABB portal_aabb{
            .pMin = pos + Vector3{
                              -consts::portalWidth / 2.f,
                              -consts::portalWidth / 2.f,
                              0.f,
                          },
            .pMax = pos + Vector3{consts::portalWidth / 2.f, consts::portalWidth / 2.f, 0.1f},
        };

        bool portal_occupied = false;
        RigidBodyPhysicsSystem::findEntitiesWithinAABB(
            ctx, portal_aabb, [&](Entity)
            { portal_occupied = true; });

        state.isOn = portal_occupied;
    }
                             ...
                             ButtonState &state)
    {
        AABB button_aabb{
            ...
            .pMax = pos + Vector3{consts::buttonWidth / 2.f, consts::buttonWidth / 2.f, 0.25f},
        };
                                          ...
                                          DoorObservation &door_obs)
    {
        ...
        self_obs.isGrabbing = grab.constraintEntity != Entity::none() ? 1.f : 0.f;
...
// environments
    ...
    void Sim::setupTasks(TaskGraphBuilder &builder, const Config &cfg)
    {
        ...
        auto move_sys = builder.addToGraph<ParallelForNode<Engine,
                                                           ...
                                                           ExternalForce,
                                                           ExternalTorque>>({});
        auto dummy_sys = builder.addToGraph<ParallelForNode<Engine, dummySystem, Rotation>>({});
